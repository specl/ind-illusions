...
dat4$JV = (dat4$J-1)*2+dat4$version
mod = lm(y ~ JV:S, data = dat4)
```
...
```{r}
library(corrplot)
plotCor = function(cors, main = " ", circ = F, clust = F, cx = 1){
...
scores = tapply(tdat$y, list(tdat$S, tdat$JV), mean, na.rm	= T)
tdat.cor = cor(scores, use = "pairwise.complete.obs")
rownames(tdat.cor) = colnames(tdat.cor) = new_names
plotCor(tdat.cor)
```
...
scores1 = tapply(dat4$y, list(dat4$S, dat4$JV), mean)
...
tdat = dat4[-influential_points,]
scores2 = tapply(tdat$y, list(tdat$S, tdat$JV), mean, na.rm	= T)
...
tdat = dat4[-influential_points2,]
scores3 = tapply(tdat$y, list(tdat$S, tdat$JV), mean, na.rm	= T)
...
```{r, fig.width=10, fig.height=7}
par(mfrow = c(1, 2), mar = c(2.1, 4.1, 3.8, 2.1))
scores1 = tapply(dat4$y, list(dat4$S, dat4$JV), mean)
scores_cor = round(cor(scores1),2)
rownames(scores_cor) = colnames(scores_cor) = new_names
plotCor(scores_cor)
title("Cleaned Data", line = 3)

dat = read.csv("../_data/processed-data.csv")
dat$S = as.numeric(factor(dat$sub))
dat$J = as.numeric(factor(dat$task))
dat$JV = (dat$J-1)*2+dat$version
scores2 = tapply(dat$y, list(dat$S, dat$JV), mean)
scores_cor = round(cor(scores2),2)
rownames(scores_cor) = colnames(scores_cor) = new_names
plotCor(scores_cor)
title("All Partip", line = 3)
```
...
```{r}
clean_data = dat4[ , c(setdiff(names(dat4), "X"))]
clean_data = clean_data[ , c("sub", setdiff(names(clean_data), "sub"))]

write.csv(clean_data, "../_data/clean_data.csv", row.names = FALSE)
```