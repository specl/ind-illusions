...
for (i in 1:5){
  for (j in 1:2){
    ...
    fit = runJags(mod = modUW, dat = data_list, pars = pars)
    post_theta[[k]] =  fit$BUGSoutput$sims.list$theta
    k = k + 1
  }
}
...
fit = runJags(mod = modUW_Contrast, dat = data_list, pars = pars)
post_theta[[11]] = fit$BUGSoutput$sims.list$theta
...
fit = runJags(mod = modUW_Contrast, dat = data_list, pars = pars)
post_theta[[12]] = fit$BUGSoutput$sims.list$theta

cols = readRDS("plot-cols.RDS")
col1 = cols$Thistle[2]
col2 = cols$Thistle[5]
col3 = "#006400"
plotTasks = function(scores, theta, task_name, y_labs, x_labs, top = 0, scale = F, g_est = NA, exp_rel_est = NA, g_pos = NA){
  ...
  # theta = post_theta[[1]]
  scores_post = apply(theta, 2, mean)
  CI = apply(theta, 2, function(x) quantile(x, probs = c(0.025, 0.975)))
  
  o = order(scores)
  tscores = scores[o]
  tscores_post = scores_post[o]
  ...
  polygon(
    c(1:I, rev(1:I)), 
    c(tCI_95[1,], rev(tCI_95[2,])), 
    col = col1, 
    border = F
  )
  lines(1:I, tscores_post, pch = 16, col = col2, lwd = 1.5, lty = 1)
  lines(1:I, tscores, pch = 16, col = col3, lwd = 1.5, lty = 6)
  ...
  text(g_pos[1], g_pos[2], text_expression, adj = 0, cex = 1)
  text_expression = bquote(italic(r) == .(round(exp_rel_est,3)))
  text(g_pos[3], g_pos[4], text_expression, adj = 0, cex = 1)
