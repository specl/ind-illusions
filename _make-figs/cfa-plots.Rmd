...
```{R}
plot_curve <- function(start, end, distance, mid_y = 0) {
  ...
  lines(curve_points, col = "blue", lwd = 2)
}

# Example usage:
start <- c(1, 1)
end <- c(1, 2)
distance <- 2  # Adjusted for more horizontal extension
plot_curve(start, end, distance)
```
...
plotCFA2 = function(){
  ...
  text(gf_pos.x, gf_pos.y, "G", cex = 2)
...
plotforme = function(){
  ...
  n.task = 10
  task_pos.y = seq(0, 2, length.out = n.task)
  task_pos.x = rep(0, n.task)
  task.length = c(.2, .05)
  n.factor_versions = 5
  version_factor.length = .3
  version_factor_pos.y = mapply(
  ...
  version_factor_pos.x = rep(1.5, n.factor_versions)
  rect(task_pos.x+task.length[1],
       task_pos.y+task.length[2],
       task_pos.x-task.length[1],
       task_pos.y-task.length[2],
  ...
  for (i in seq_along(version_factor_pos.x)) {
    # Calculate ellipse bounding box
    
    # Plot ellipse
    symbols(
      x = version_factor_pos.x[i],
      y = version_factor_pos.y[i],
      circles = 1,  # Just one circle to represent the ellipse
      inches = FALSE,
      add = TRUE,  # Background color of ellipse
      fg = "pink"   # Border color of ellipse (same as background for a filled ellipse)
    )
  }
}
